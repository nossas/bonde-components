...
const CommunitiesDropdown = ({ community: defaultCommunity }: any) => {
  const { community, communities, onChangeCommunity } = useSession(defaultCommunity);
...
const Routing = () => {
  ...
  return (
    <Main>
      ...
      >
        {pathname === '/with-menu' ? <CommunitiesDropdown community={community} /> : null}
      </Navbar>
      <Body>
        <Switch>
          <Route exact path='/'>
          ...
          <Route exact path='/with-menu'>
            <Text>{!community ? 'Select your community' : `Welcome to ${community.label}`}</Tex>
          </Route>
        </Switch>
